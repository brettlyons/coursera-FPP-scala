...
object Huffman {
  ...
  /**
    ...
    */
  // could probably use List[a] here.
  def until(endPredFn: List[CodeTree] => Boolean, applyFn: List[CodeTree] => List[CodeTree])(mainTree: List[CodeTree]): List[CodeTree] = {
    if(endPredFn(mainTree))
      mainTree
    else
      until(endPredFn, applyFn)(applyFn(mainTree))
  }
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    // def times(chars: List[Char]): List[(Char, Int)] = {
    // def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    val tmpFreqsList = times(chars);
    val ordLeaves = makeOrderedLeafList(tmpFreqsList);
    (until(singleton, combine)(ordLeaves)).head
  }
